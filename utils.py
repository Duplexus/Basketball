...
def init_data_loaders(trainset, valset, testset, num_cpus, batch_size=1024):
...
def run_training(model, optimizer, loss_function, device, num_epochs, 
    ...
    for epoch in master_bar:
        ...
        ## YOUR CODE HERE ##
        ####################
        # custom code for early stopper
        if not early_stopper == None:
            early_stopper.update_patience_spent(val_accs, epoch, model)

            if early_stopper.patience_spent > early_stopper.patience:
...
def diag():
    print(f"overall highest accuracy in epoch {np.argmax(val_accs)}, value: {np.max(val_accs)}")
    print(f"overall lowest loss in epoch {np.argmin(val_losses)}, value: {np.min(val_losses)}")
